...
class Example(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('Отсечение многоугольников')
        ...
        self.rb1.setText("Ввод многоугольника")
        ...
        self.tab.addTab(self.table, "Многоугольник")
    ...
    def cut_convex(self, edges, cutter):
        ...
        resPol = []
        resPol = cut_polygon(pol=edges, cutter=cutter,
                             normVect=normVect, resPol=resPol)
        for i in range(len(resPol) - 1):
            self.Bresenham(resPol[i][0], resPol[i][1],
                           resPol[i + 1][0], resPol[i + 1][1], self.colormain, True)
        self.repaint()
        # time.sleep(1)
        # print(resPol)
    ...
    def Cut(self):
        ...
        if len(self.edges) == 0:
            ...
            self.ShowWarning("Не задан многоугольник!")
            return

        convex_flag = check_convex(self.edges)
        if convex_flag == 0:
            self.ShowWarning("Заданный отсекатель вырожден в отрезок!")
            return
        elif convex_flag == -1:
            self.ShowWarning("Задан невыпуклый отсекатель!")
            return
    ...
    def draw_borders(self):
        if len(self.edges_slave) > 1:
            for i in range(len(self.edges_slave) - 1):
                self.Bresenham(self.edges_slave[i][0],
                               self.edges_slave[i][1],
                               self.edges_slave[i + 1][0],
                               self.edges_slave[i + 1][1],
                               self.colorhelp)
            ...
            if (QMouseEvent.button() == Qt.LeftButton):
                ...
                if (x >= 10 and y >= 10 and y <= HIGHT and x <= WIDTH):
                    ...
                    for i in range(len(self.edges)):
                        temp_x, temp_y = get_norm_dot(x, y, self.edges[i],
                                                      self.edges[(i + 1) % len(self.edges)])

                        dist = get_norm(x, y, self.edges[i],
                                        self.edges[(i + 1) % len(self.edges)])
                    ...
                    i = len(self.edges_slave)

                    if self.capslock and i:
                        if y != self.edges_slave[i - 1][1]:
                            der = ((x - self.edges_slave[i - 1][0]) /
                                   (y - self.edges_slave[i - 1][1]))
                        else:
                            der = 2
                        if abs(der) <= 1:
                            x = self.edges_slave[i - 1][0]
                        else:
                            y = self.edges_slave[i - 1][1]

                    if i:
                        self.Bresenham(self.edges_slave[i - 1][0],
                                       self.edges_slave[i - 1][1],
                                       x, y, self.colorhelp)
                    self.edges_slave.append([x, y, 0])
                    self.table_appender([x, y])

            elif (QMouseEvent.button() == Qt.RightButton):
                i = len(self.edges_slave)
                if i:
                    x = self.edges_slave[0][0]
                    y = self.edges_slave[0][1]
                    self.Bresenham(self.edges_slave[i - 1][0],
                                   self.edges_slave[i - 1][1],
                                   x, y, self.colorhelp)
                    self.edges_slave.append([x, y, 0])
