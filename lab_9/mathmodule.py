...
def P(t, p1, p2): # вычисление точки пересечения окна и многоугольника
    tmp = [0, 0, 0]
...
def get_line_coefs(x1, y1, x2, y2): #для отрисовки
    return ((y1 - y2), (x2 - x1), (x1 * y2 - x2 * y1))


def get_norm_dot(x, y, p1, p2): #для отрисовки
...
def get_norm(x, y, p1, p2): #для отрисовки
    a, b, c = get_line_coefs(p1[0], p1[1], p2[0], p2[1])
...
def is_visible(p1, c1, norm): # определяем видимость точки
    vec = diff_vec(p1, c1) # вектор W
    res = mul_scalar(vec, norm) # скалярное произведение W и нормали
    return res > 0


def cut_polygon(pol, cutter, normVect, resPol):
    n = len(cutter) - 1
    F = 0 # запоминаем первую вершину, чтобы посмотреть последнее ребро
    S = 0 # какая-нибудь вершина многоугольника
    I = 0
    Dsk = 0
    Wsk = 0
    t = 0
